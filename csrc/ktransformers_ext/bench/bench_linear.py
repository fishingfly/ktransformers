...
def bench_linear(quant_mode: str):
    with torch.inference_mode(mode=True):
        ...
        for _ in range(layer_num):
            proj = torch.randn((output_size, input_size), dtype=torch.float32, device = "musa").to("cpu").contiguous()
        ...
        input = torch.randn((layer_num, qlen, input_size), dtype=torch.bfloat16, device = "musa").to("cpu").contiguous()
        output = torch.empty((layer_num, qlen, output_size), dtype=torch.bfloat16, device = "musa").to("cpu").contiguous()
