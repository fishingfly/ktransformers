...
class GGUFLoader:
    ...
    def load_expert_tensor(self, name, data, expert_id, elements_per_expert, device = "musa", target_dtype = torch.get_default_dtype())->torch.Tensor:
        t = self.tensor_info[name]
        if device.lower() == "cpu":
...
def dequantize_q2_k_gpu(data, device:str ="musa", target_dtype = torch.get_default_dtype()):
    block_size = GGML_BLOCK_SIZES["Q2_K"]
...
def dequantize_q3_k_gpu(data, device:str ="musa", target_dtype = torch.get_default_dtype()):
    block_size = GGML_BLOCK_SIZES["Q3_K"]
...
def dequantize_q4_k_gpu(data, device:str ="musa", target_dtype = torch.get_default_dtype()):
    block_size = GGML_BLOCK_SIZES["Q4_K"]
...
def dequantize_q5_k_gpu(data, device:str ="musa", target_dtype = torch.get_default_dtype()):
    block_size = GGML_BLOCK_SIZES["Q5_K"]
...
# @torch.jit.script
def dequantize_q6_k_gpu(data: np.ndarray, device:str = "musa", target_dtype = torch.get_default_dtype()):
    block_size = GGML_BLOCK_SIZES["Q6_K"]
...
def dequantize_iq4_xs_gpu(data: np.ndarray, device:str = "musa", target_dtype = torch.get_default_dtype()):
    block_size = GGML_BLOCK_SIZES["IQ4_XS"]
...
def dequantize_q4_0_gpu(data, device:str = "musa", target_dtype = torch.get_default_dtype()):
...
def dequantize_q5_0_gpu(data, device:str = "musa", target_dtype = torch.get_default_dtype()):
    raise NotImplementedError()
...
def dequantize_q8_0_gpu(data, device:str = "musa", target_dtype = torch.get_default_dtype()):
    # C struct definition
    # https://github.com/ggerganov/ggml/blob/fca1caafea7de9fbd7efc733b9818f9cf2da3050/src/ggml-quants.h#L43
